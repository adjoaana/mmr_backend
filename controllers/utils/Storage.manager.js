...
class StorageManager {
	...
	upload = async (file, folder_path = "") => {
		...
		const uploadParams = {
			...
			Body: fileStream,
			Key: folder_path + (process.env.NODE_ENV === "test" || process.env.NODE_ENV === "dev" ? "test_" : "") + file.filename,
		}
	...
	getPresignedUrl = async (key, fileType) => {
		...
		const response = await this.s3.getSignedUrlPromise('putObject', uploadParams);
		return response;
	}
