...
class TemplateController extends Controller {
     ...
     getOne = async(req, res)=> {
		...
		try {
			...
			const thumbImage = await this.storageManager.getFileUrl(response.thumbImage.key);
			response = {
				_id: response._id,
    ...
    addExercises = async (req, res) => {
     ...
     getWarmupExercises = async (req, res) => {
		...
		try {
			...
			if (response.warmup) {
				response = await Promise.all(response.warmup.map(async item => {
    ...
    getMainExercises = async (req, res) => {
		...
		try {
			...
			if (response.main) {
				response = await Promise.all(response.main.map(async item => {
     ...
     getCooldownExercises = async (req, res) => {
		...
		try {
			...
			if (response.cooldown) {
				response = await Promise.all(response.cooldown.map(async item => {
    ...
    updateOneUserTemplate = async (req, res) => {
		...
		try {
			...
			const body = req.body;
			const user_id = req.req_user._id
			const response = await this.service.updateOneUserTemplate(user_id, id, body);
			res.status(200).json({
				status: 200,
				body: response,
				route: this.routeString
