...
const dbOptions = { useNewUrlParser: true, useUnifiedTopology: true, ssl: true };//ssl: process.env.NODE_ENV === "prod" ? true : false};
...
if (process.env.NODE_ENV !== "test") {
    ...
    mongoose.connect(config.DB_URL,
        ...
        (err) => {
            if (err)
                ...
                automationManager.registerTask(30, appointmentService.rejectOverdueAppointments());
    ...
    app.use(morgan('mmr - [:date[clf]] :remote-addr - :remote-user  ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" ":user-agent"')); // common | combined
}
...
// Welcome message
app.get('/', (req, res) => { res.json({ route: "MonitorMyRehab API", message: "Hello there. Are you on aws or where are you?\nThis is the monitorMyRehab API version 1.0.0" }); });

app.post("*", (req, res) => { res.status(404).json({ message: 'Endpoint not supported', status: 404 }); })
app.put("*", (req, res) => { res.status(404).json({ message: 'Endpoint not supported', status: 404 }); })
app.delete("*", (req, res) => { res.status(404).json({ message: 'Endpoint not supported', status: 404 }); })
